...
impl Anaconda {
    pub fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        for line in lines {
            if line.contains("anaconda3/bin") {
                ...
                let processed_line = line.strip_suffix("/bin").unwrap();
    ...
    /// get the environment path for Unix platforms
    pub fn get_environment_path(
	    ...
	    if !cfg!(target_os = "windows") {
	        environment_path = self.path.clone()
	            + "/envs/"
	            + environment.as_str()
	            + "/lib/"
    ...
    pub fn get_packages(
        ...
        // read the packages directory for retrieving them
        let package_jsons = std::fs::read_dir(environment_path)?;

        // find out dist-info
        let mut dist_infos: Vec<String> = Vec::new();
        for package_json in package_jsons {
            let package = package_json.unwrap().file_name();

            let elements_string = package.to_string_lossy();
            let mut elements: Vec<&str> = elements_string.split(".").collect();
        ...
        for item in dist_infos {
            ...
            let package_name = filename.get(0).unwrap();

            let dist_info_unstripped = filename.get(1).unwrap();
            let strings: Vec<&str> = dist_info_unstripped.split(".").collect();
            ...
            let package_version = dist_info_stripped.join(".");
...
impl CondaEnvironment {
    pub fn new(environment_name: String) -> Result<Self, Box<dyn std::error::Error>> {
...
pub fn lookup_conda_environments() -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let output = std::process::Command::new("conda")
        .arg("info")
        .arg("--envs")
        .output()
